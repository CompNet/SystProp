...
#		367,17,431,27,369,370,371,304,334,391,318,611,65,66,
#		312,315,479,313,390,372,482,301,192,299,373,478,321,343,
#		317,109,314,344,345,293,385,220,428,412,59,462,355,453,
#		472,323,333,302,480,341,149,320,376,190,466,307,365,295,
#		425,72,450,455,150,438,439,298,427,447,407,274,359,311,
#		398,399,400,401,395,421,468,422,406,405,303,434,454,300,397,
#		394,429,430,393,388,335,435,396,469,
		470,389,297,18,58,420,55,
...
{	gc()
	...
	{	# setup file name
		...
		{	# load network
			...
			# estimate the lower cut-off value
				...
				{	m$setXmin(x.min)
					...
					{	m$setPars(alpha)
						...
						p.val <- tryCatch(expr=bootstrap_p(m, no_of_sims=100, threads=nb.cores)$p, error=function(e) NA)
					}
				}
			# init result matrix
			...
			out.file <- paste(net.folder,comparison.file,sep="") 
